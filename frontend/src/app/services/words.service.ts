...
export class WordsService {
    ...
    saveWord(id: string, body: any){
        ...
        if(token){
            ...
            return this.http.post(environment.uri + "/word/" + id, body, {headers});
        } else {
            return this.http.post(environment.uri + "/word/" + id, body);
        }
    }
    ...
    getWord(id: string, wordID: string){
        ...
        if(token){
            const headers: HttpHeaders = new HttpHeaders().set("Authorization", 'Bearer ' + token);
            return this.http.get(environment.uri + "/word/id/" + id + "/" + wordID, {headers});
        } else {
            return this.http.get(environment.uri + "/word/id/" + id + "/" + wordID);
        }
    }
